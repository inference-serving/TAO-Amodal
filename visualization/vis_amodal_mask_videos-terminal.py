...
def main():
    ...
    * Create Dataset to load videos
    '''
    coco = COCO(args.annotations) # TODO HERE
    ...
    * Select the videos we need
    ...
    tasks = []
    for video, labeled_frames in videos.items(): # TODO labeled frames, where does it come from
        output_video = args.output_dir / (video + '.mp4')
    ...
    * Visualization with Multi-Processing
    ...
    #               total=len(tasks)))
    # Always process tasks sequentially
    for task in tqdm(tasks):
        visualize(*task)
